...
import {
  Zap, Check, Sparkles, Newspaper, Trophy, BookOpen, X,
} from "lucide-react";
...
function PremiumContent() {
  ...
  useEffect(() => {
    fetch("/api/v1/auth/session")
      .then((r) => r.json())
      .then(async ({ data: s }) => {
        if (!s?.user) { setIsPremium(false); return; }
        const res = await fetch("/api/v1/user/stats"); // reuse auth check
        if (res.status === 401) { setIsPremium(false); return; }

        // Check subscription
        const subRes = await fetch("/api/v1/user/subscription");
        if (subRes.ok) {
      ...
      <main className="max-w-lg mx-auto px-5 pb-24 pt-8">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-[var(--text-secondary)]">
            Smarter news. Less noise. All yours.
          </p>
        </div>
        ...
        {!isPremium && (
          ...
          >
            {loading ? "Redirecting to checkout…" : `Subscribe — ${PLANS.find((p) => p.key === selectedPlan)?.price}${PLANS.find((p) => p.key === selectedPlan)?.period}`}
          </button>
        )}
